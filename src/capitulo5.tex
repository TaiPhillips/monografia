...
deve aprender (número ponto-flutuante de 0 à 1), mas é codificado como um
inteiro de
...
equação \ref{eq:funcao_a_modelar}, que a rede neural precisa aprender a
...
(\emph{breadth-first search}), no no qual a ordem na
...
que o bloco ``NN1'', que implementa a rede neural seja o mais otimizado
possível para classificar imagens, não tanto para treinamento.
...
vai ocorrer em paralelo. Quando 2560 imagens são lidas elas são embaralhadas
...
imagens que é parte do pipeline de leitura, uma mesma imagem passará novamente
pelo pipeline de distorções, efetivamente gerando uma imagem diferente.
...
\begin{figure}[!htb]
	...
	\caption{Pipeline descrevendo a rede neural convolucional profunda}
	\label{fig:cap5_cnn}
	(próprio autor)
\end{figure}
...
A saída da rede neural, denominado ``A'', é um tensor unidimensional com
dimensão 256, contendo um escalar que representa o valor estimado da função
para uma imagem. Para o treinamento a saída
da rede neural vai ser comparado com os rótulos que estavam nos dados de
...
um único escalar, denominado perda, ou \emph{loss}, pela metade da normal
...
\begin{figure}[!htb]
	...
	se deve maior quantidade de dados no set de treinamento, o que gera menos
	incerteza (próprio autor).
\end{figure}
...
\emph{frames} onde veículos se encontrem nesta região.
