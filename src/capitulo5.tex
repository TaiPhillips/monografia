...
várias vezes, mas o resultado final, conforme visto no player, continuava ruim.
...
A versão final da DCNN usa o \emph{framework} TensorFlow do Google.
No TensorFlow usa-se a linguagem Python para descrever um ou mais
grafos onde
...
computadores usando GPUs de alto desempenho. O TensorFlow permite usar Python
2 ou Python 3, sendo que a versão 3 foi escolhida para o projeto.
...
Tanto o \emph{wrapper} Python do OpenCV quanto o TensorFlow representam dados
numéricos, como tensores, usando uma biblioteca numérica para Python chamada
\emph{numpy}. Ela foi usada para várias operações, como extrair sub-imagens
...
onde estão as placas, foi usado o formato \sigla{JSON}{Javascript
object notation}. Este formato está se tornado o padrão de facto para
representação de dados no mercado. Além disso existem boas
...
A CPU do computador Core i7, quando usado para treinamento, rapidamente
...
A capacidade de produzir estes gráficos foi incluído no software "player", e
pode ser invocada para qualquer frame de qualquer vídeo.
...
tempo gasto em cada tarefa foi gerado o gráfico na figura
\ref{fig:cap5_frame_benchmark}.
