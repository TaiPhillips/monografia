...
filtros lineares, existem algumas opções no tratamento de bordas dos dados.
...
não-convolucionais podem ser aplicadas. No entanto a função ReLu, ou linear
...
aparecer, e
fazer isso para todas as \emph{features}.
...
detector à imagem usando um operador de correlação cruzada em
duas dimensões. Se o
...
Como em uma correlação cruzada, assim como na convolução, cada número de um dos
...
\begin{figure}[!htb]
	...
	correlação cruzda é multiplicado exatamente uma vez por cada
	um dos termos do outro operando, resultando em um número de
	multiplicações igual ao produto do número de termos em cada operando
	(próprio autor).
\end{figure}

A consequência disso é que quando se calcula a correlação cruzada entre duas
